...
//chat support
// Enhanced ChatSupport class with proactive features
class ChatSupport {
    constructor() {
        ...
        this.proactiveEnabled = this.widget.dataset.proactive === 'true';
        this.proactiveDelay = parseInt(this.widget.dataset.delay) || 10000;
        this.pageViewTime = 0;
        this.sectionViewedTimes = {};
        ...
        this.proactiveMessages = {
            'rooms': 'I notice you\'re looking at our rooms. Would you like help choosing the perfect room for your stay?',
            'special-offers': 'I see you\'re checking our special offers. Would you like help booking one of these special deals?',
            'reservation-intent': 'Looking to make a reservation? I can help you with that!',
            'long-browsing': 'Is there anything specific you\'re looking for? I\'m here to help!',
            'exit-intent': 'Before you go, would you like to know about our current special offers?'
        };

        this.initializeEventListeners();
        this.initializeTracking();
        
        // Start proactive features if enabled
    ...
    initializeTracking() {
        ...
        setInterval(() => {
            this.pageViewTime = (Date.now() - this.pageViewStart) / 1000;
        }, 1000);
        
        // Track sections being viewed
        ...
        this.trackReservationIntent();
    }
    
    initializeProactiveChat() {
        // Initial delay before showing chat prompt
        setTimeout(() => {
            if (!this.hasInteracted && !this.isOpen) {
                this.showProactiveMessage('long-browsing');
            }
        }, this.proactiveDelay);
    }
    
    observeSections() {
        // Observe which sections the user is viewing
            ...
            const sectionObserver = new IntersectionObserver((entries, observer) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        ...
                        if (!this.sectionViewedTimes[sectionId]) {
                            this.sectionViewedTimes[sectionId] = Date.now();
                        }
                        
                        // If user spends more than 10 seconds on a section, show relevant proactive message
                        setTimeout(() => {
                            const timeViewing = (Date.now() - this.sectionViewedTimes[sectionId]) / 1000;
                            if (timeViewing > 10 && !this.hasInteracted && !this.isOpen) {
                                if (sectionId === 'rooms') {
                                    this.showProactiveMessage('rooms');
                                } else if (sectionId === 'special-offers') {
                                    this.showProactiveMessage('special-offers');
                                }
                            }
                        }, 10000);
                    }
    ...
    trackExitIntent() {
        ...
        document.addEventListener('mouseleave', (e) => {
            if (e.clientY <= 0 && !this.hasInteracted && !this.isOpen && this.pageViewTime > 10) {
                this.showProactiveMessage('exit-intent');
            }
        });
    }
    
    trackReservationIntent() {
        // Track clicks on booking-related elements
        const bookingElements = document.querySelectorAll('.cta-button, .offer-button, a[href*="reservation"]');
        bookingElements.forEach(element => {
            element.addEventListener('click', () => {
                ...
                setTimeout(() => {
                    if (!this.hasInteracted && !this.isOpen) {
                        this.showProactiveMessage('reservation-intent');
                    }
                }, 2000);
            });
        });
    ...
    showProactiveMessage(messageType) {
        // Add a notification badge with the message
        this.badge.style.display = 'flex';
        this.badge.textContent = '1';
        
        // Store the message type to show when chat is opened
        this.pendingProactiveMessage = messageType;
        
        // Animate the chat button to draw attention
        this.trigger.classList.add('pulse-animation');
        
        // Add click handler to show the specific message
        const originalClickHandler = this.trigger.onclick;
        this.trigger.onclick = () => {
            this.toggleChat();
            this.hasInteracted = true;
            
            // Remove the animation
            this.trigger.classList.remove('pulse-animation');
            
            // Show the proactive message
            setTimeout(() => {
                this.addMessage(this.proactiveMessages[this.pendingProactiveMessage], false);
                this.pendingProactiveMessage = null;
            }, 500);
            
            // Restore original click handler
            this.trigger.onclick = originalClickHandler;
        };
    }
    ...
    sendMessage() {
        ...
        setTimeout(() => {
            ...
            this.suggestFollowupQuestions(message, response);
        }, 500);
    }
    
    suggestFollowupQuestions(userMessage, botResponse) {
        let suggestions = [];
        
        // Generate suggestions based on current conversation context
        if (userMessage.toLowerCase().includes('room') || botResponse.includes('room')) {
            suggestions.push('What amenities are included?', 'What\'s the price?');
        } else if (userMessage.toLowerCase().includes('book') || botResponse.includes('book')) {
            suggestions.push('Do you have any special offers?', 'What\'s your cancellation policy?');
        } else if (userMessage.toLowerCase().includes('location') || botResponse.includes('location')) {
            suggestions.push('How far is it from the airport?', 'Is parking available?');
        }
        
        // Only show suggestions if we have any
        if (suggestions.length > 0) {
            // Add a slight delay before showing suggestions
            setTimeout(() => {
